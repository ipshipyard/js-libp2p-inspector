...
export interface FindProvidersProps {
  metrics: MetricsRPC
}

export interface FindProvidersState {
  target: string
  error: string
  details: JSX.Element[]
}

export class FindProviders extends Component<FindProvidersProps, FindProvidersState> {
  constructor (props: FindProvidersProps) {
    super(props)
  ...
  private findProviders (evt: FormEvent | Event, target: string): boolean {
    evt.preventDefault()
  ...
  render () {
    return (
      <Panel>
        <p>Enter a CID to find providers of:</p>
        <form onSubmit={(evt) => this.findProviders(evt, this.state.target)}>
          <TextInput type="text" value={this.state.target} placeholder="bafyfoo..." onChange={(e) => {
            this.setState({
              target: e.target.value
            })
          }} />
          <Button onClick={(evt) => this.findProviders(evt, this.state.target)} primary={true}>Get</Button>
        </form>
